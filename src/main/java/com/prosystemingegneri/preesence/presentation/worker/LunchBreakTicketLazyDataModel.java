 ...
 */
public class LunchBreakTicketLazyDataModel extends LazyDataModel<LunchBreakTicket>{
    private final LunchBreakTicketService service;

    public LunchBreakTicketLazyDataModel(LunchBreakTicketService service) {
        this.service = service;
    }
    
    @Override
    public Object getRowKey(LunchBreakTicket object) {
        return object.getId();
    }
    
    @Override
    public List<LunchBreakTicket> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, FilterMeta> filterBy) {
        String name = getStringFromFilter(filterBy, "name");
        
        List<LunchBreakTicket> result = service.list(first, pageSize, sortField, getAscending(sortOrder), name);
    ...
    @Override
    public LunchBreakTicket getRowData(String rowKey) {
        try {
